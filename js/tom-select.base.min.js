...
for(const i of e.children)"optgroup"===(t=i.tagName.toLowerCase())?f(i):"option"===t&&v(i)})():(()=>{var t,r,l=e.getAttribute(s)
...
if(o=d.settings.mode,r=u.getAttribute("class")||"",e=g("<div>"),m(e,p.wrapperClass,r,o),t=g('<div class="items">'),m(t,p.inputClass),e.append(t),s=d._render("dropdown"),m(s,p.dropdownClass,o),n=g(`<div role="listbox" id="${v}" tabindex="-1">`),m(n,p.dropdownContentClass),s.append(n),g(p.dropdownParent||e).appendChild(s),p.controlInput)i=g(p.controlInput)
...
for(O&&(c=O.dataset.value,d=O.closest("[data-group]")),n=m.items.length,"number"==typeof v.settings.maxOptions&&(n=Math.min(n,v.settings.maxOptions)),n>0&&(b=!0),t=0;t<n;t++){let e=v.options[m.items[t].id],n=P(e[v.settings.valueField]),l=v.getOption(n)
...
if(v.settings.shouldLoad.call(v,f)?v.loading?I("loading"):0===m.items.length&&I("no_results"):I("not_loading"),(a=v.canCreate(f))&&(p=I("option_create")),v.hasOptions=m.items.length>0||a,b){if(m.items.length>0){if(!w.contains(O)&&"single"===v.settings.mode&&v.items.length&&(O=v.getOption(v.items[0])),!w.contains(O)){let e=0
...
e.isLocked||e.isOpen||"multi"===e.settings.mode&&e.isFull()||(e.isOpen=!0,_(e.control_input,{"aria-expanded":"true"}),e.refreshState(),f(e.dropdown,{visibility:"hidden",display:"block"}),e.positionDropdown(),f(e.dropdown,{visibility:"visible",display:"block"}),e.focus(),e.trigger("dropdown_open",e.dropdown))}close(e=!0){var t=this,i=t.isOpen
e&&(t.setTextboxValue(),"single"===t.settings.mode&&t.items.length&&(t.hideInput(),t.tab_key||t.blur())),t.isOpen=!1,_(t.control_input,{"aria-expanded":"false"}),f(t.dropdown,{display:"none"}),t.settings.hideSelected&&t.clearActiveOption(),t.refreshState(),i&&t.trigger("dropdown_close",t.dropdown)}positionDropdown(){if("body"===this.settings.dropdownParent){var e=this.control,t=e.getBoundingClientRect(),i=e.offsetHeight+t.top+window.scrollY,s=t.left+window.scrollX
