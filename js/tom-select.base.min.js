...
s.tomselect=this,i=(window.getComputedStyle&&window.getComputedStyle(s,null)).getPropertyValue("direction"),t=G(s,t),this.settings=t,this.input=s,this.tabIndex=s.tabIndex||0,this.is_select_tag="select"===s.tagName.toLowerCase(),this.rtl=/rtl/i.test(i),this.inputId=M(s,"tomselect-"+J),this.isRequired=s.required,this.sifter=new w(this.options,{diacritics:t.diacritics}),t.mode=t.mode||(1===t.maxItems?"single":"multi"),"boolean"!=typeof t.hideSelected&&(t.hideSelected="multi"===t.mode),"boolean"!=typeof t.hidePlaceholder&&(t.hidePlaceholder="multi"!==t.mode)
...
if(v.loading?I("loading"):v.settings.shouldLoad.call(v,m)?0===f.items.length&&I("no_results"):I("not_loading"),(a=v.canCreate(m))&&(p=I("option_create")),v.hasOptions=f.items.length>0||a,w){if(f.items.length>0){if(!b.contains(O)&&"single"===v.settings.mode&&v.items.length&&(O=v.getOption(v.items[0])),!b.contains(O)){let e=0
