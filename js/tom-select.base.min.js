...
return!(e&&-1!==s.items.indexOf(e))}))),e}refreshOptions(t=!0){var e,i,s,n,o,r,l,a,c,d,u
const p={},h=[]
var g,f=this,v=f.inputValue(),m=f.search(v),y=null,O=f.settings.shouldOpen||!1,b=f.dropdown_content
for(f.activeOption&&(c=f.activeOption.dataset.value,d=f.activeOption.closest("[data-group]")),n=m.items.length,"number"==typeof f.settings.maxOptions&&(n=Math.min(n,f.settings.maxOptions)),n>0&&(O=!0),e=0;e<n;e++){let t=m.items[e]
if(!t)continue
let n=t.id,l=f.options[n]
if(void 0===l)continue
let a=W(n),u=f.getOption(a,!0)
...
let t=p[o]
void 0===t&&(t=document.createDocumentFragment(),h.push(o)),i>0&&(u=u.cloneNode(!0),B(u,{id:l.$id+"-clone-"+i,"aria-selected":null}),u.classList.add("ts-cloned"),D(u,"active")),y||c!=n||(d?d.dataset.group===o.toString()&&(y=u):y=u),t.appendChild(u),p[o]=t}}f.settings.lockOptgroupOrder&&h.sort(((t,e)=>{const i=f.optgroups[t],s=f.optgroups[e]
return(i&&i.$order||0)-(s&&s.$order||0)})),l=document.createDocumentFragment(),L(h,(t=>{let e=p[t]
if(!e||!e.children.length)return
let i=f.optgroups[t]
if(void 0!==i){let t=document.createDocumentFragment(),s=f.render("optgroup_header",i)
ot(t,s),ot(t,e)
let n=f.render("optgroup",{group:i,options:t})
ot(l,n)}else ot(l,e)})),b.innerHTML="",ot(b,l),f.settings.highlight&&(g=b.querySelectorAll("span.highlight"),Array.prototype.forEach.call(g,(function(t){var e=t.parentNode
e.replaceChild(t.firstChild,t),e.normalize()})),m.query.length&&m.tokens.length&&L(m.tokens,(t=>{Q(b,t.regex)})))
var w=t=>{let e=f.render(t,{input:v})
return e&&(O=!0,b.insertBefore(e,b.firstChild)),e}
if(f.loading?w("loading"):f.settings.shouldLoad.call(f,v)?0===m.items.length&&w("no_results"):w("not_loading"),(a=f.canCreate(v))&&(u=w("option_create")),f.hasOptions=m.items.length>0||a,O){if(m.items.length>0){if(y||"single"!==f.settings.mode||null==f.items[0]||(y=f.getOption(f.items[0])),!b.contains(y)){let t=0
u&&!f.settings.addPrecedence&&(t=1),y=f.selectable()[t]}}else u&&(y=u)
t&&!f.isOpen&&(f.open(),f.scrollToOption(y,"auto")),f.setActiveOption(y)}else f.clearActiveOption(),t&&f.isOpen&&f.close(!1)}selectable(){return this.dropdown_content.querySelectorAll("[data-selectable]")}addOption(t,e=!1){const i=this
...
function l(t,i,s){return t||(t=T('<option value="'+X(i)+'">'+X(s)+"</option>")),t!=n&&e.input.append(t),o.push(t),(t!=n||r>0)&&(t.selected=!0),t}e.input.querySelectorAll("option:checked").forEach((t=>{t.selected=!1})),0==e.items.length&&"single"==e.settings.mode?l(n,"",""):e.items.forEach((t=>{if(i=e.options[t],s=i[e.settings.labelField]||"",o.includes(i.$option)){l(e.input.querySelector(`option[value="${nt(t)}"]:not(:checked)`),t,s)}else i.$option=l(i.$option,t,s)}))}else e.input.value=e.getValue()
