...
t&&t.setAttribute("for",a+"-tomselected")}if(p.settings.copyClassesToDropdown&&O(s,o),e.style.width=c.style.width,p.plugins.names.length&&(l="plugin-"+p.plugins.names.join(" plugin-"),O([e,s],l)),(null===d.maxItems||d.maxItems>1)&&p.is_select_tag&&c.setAttribute("multiple","multiple"),p.settings.placeholder&&i.setAttribute("placeholder",d.placeholder),!p.settings.splitOn&&p.settings.delimiter){var m=p.settings.delimiter.replace(/[-\/\\^$*+?.()|[\]{}]/g,"\\$&")
p.settings.splitOn=new RegExp("\\s*"+m+"+\\s*")}p.control=t,p.control_input=i,p.wrapper=e,p.dropdown=s,p.dropdown_content=n,p.control_input.type=c.type,g(s,"mouseenter",(e=>{var t=I(e.target,"[data-selectable]",s)
...
var y=e=>{var t=I(e.target,"[data-selectable]",p.dropdown)
t||p.wrapper.contains(e.target)?(h(e,!0),t&&p.onOptionSelect(e,t)):p.isFocused&&p.blur(e.target)},w=()=>{p.isOpen&&p.positionDropdown()},b=()=>{p.ignoreHover=!1}
g(document,"mousedown",y),g(window,"sroll",w,u),g(window,"resize",w,u),g(window,"mousemove",b,u),p._destroy=()=>{document.removeEventListener("mousedown",y),window.removeEventListener("mousemove",b),window.removeEventListener("sroll",w),window.removeEventListener("resize",w)}
...
t.value!==e&&(t.value=e,m(t,"update"),this.lastValue=e)}getValue(){return this.is_select_tag&&this.input.hasAttribute("multiple")?this.items:this.items.join(this.settings.delimiter)}setValue(e,t){u(this,t?[]:["change"],(()=>{this.clear(t),this.addItems(e,t)}))}setMaxItems(e){0===e&&(e=null),this.settings.maxItems=e,this.refreshState()}setActiveItem(e,t){var i,s,n,r,o,a,p=this
...
if(s=e&&8===e.keyCode?-1:1,n={start:(l=a.control_input).selectionStart,length:l.selectionEnd-l.selectionStart},r=[],a.activeItems.length){for(o=S(A(a.activeItems,s)),s>0&&o++,t=0,i=a.activeItems.length;t<i;t++)r.push(a.activeItems[t].dataset.value)
