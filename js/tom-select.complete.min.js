...
if(o=d.settings.mode,r=c.getAttribute("class")||"",w(e=y("<div>"),p.wrapperClass,r,o),w(t=y('<div class="items">'),p.inputClass),e.append(t),w(s=d.render("dropdown"),p.dropdownClass,o),w(n=y(`<div role="listbox" id="${f}" tabindex="-1">`),p.dropdownContentClass),s.append(n),y(p.dropdownParent||e).appendChild(s),p.controlInput)i=y(p.controlInput)
...
d.settings.splitOn=new RegExp("\\s*"+b+"+\\s*")}var I,C,_
...
h(e,!0),t&&d.onOptionSelect(e,t)},x=()=>{d.isOpen&&d.positionDropdown()}
g(document,"mousedown",S),g(window,"sroll",x,u),g(window,"resize",x,u),d._destroy=()=>{document.removeEventListener("mousedown",S),window.removeEventListener("sroll",x),window.removeEventListener("resize",x)},this.revertSettings={innerHTML:c.innerHTML,tabIndex:c.tabIndex},c.tabIndex=-1,k(c,{hidden:"hidden"}),c.insertAdjacentElement("afterend",d.wrapper),d.setValue(p.items),delete p.items,g(c,"invalid",(e=>{h(e),d.isInvalid||(d.isInvalid=!0,d.refreshState())})),d.updateOriginalInput(),d.refreshItems(),d.refreshState(),d.inputState(),d.isSetup=!0,c.disabled&&d.disable(),d.on("change",this.onChange),w(c,"tomselected"),d.trigger("initialize"),!0===p.preload&&d.load("")}setupOptions(e=[],t=[]){for(const t of e)this.registerOption(t)
...
if(v.settings.shouldLoad.call(v,m)?v.loading?w("loading"):0===y.items.length&&w("no_results"):w("not_loading"),(c=v.canCreate(m))&&(h=w("option_create")),v.hasOptions=y.items.length>0||c,b){if(y.items.length>0){if(!(u=O&&v.getOption(O))||!v.dropdown_content.contains(u)){let e=0
...
n=y('<option value="'+c(i)+'">'+c(e)+"</option>"),s.options[i].$option=n}n.selected=!0,k(n,{selected:"true"}),s.input.prepend(n)}else s.input.value=s.getValue()
s.isSetup&&(e.silent||s.trigger("change",s.getValue()))}open(){var e=this
e.isLocked||e.isOpen||"multi"===e.settings.mode&&e.isFull()||(e.isOpen=!0,k(e.control_input,{"aria-expanded":"true"}),e.refreshState(),b(e.dropdown,{visibility:"hidden",display:"block"}),e.positionDropdown(),b(e.dropdown,{visibility:"visible",display:"block"}),e.focus(),e.trigger("dropdown_open",e.dropdown))}close(e=!0){var t=this,i=t.isOpen
e&&t.setTextboxValue(),"single"===t.settings.mode&&t.items.length&&(t.hideInput(),t.tab_key||t.blur()),t.isOpen=!1,k(t.control_input,{"aria-expanded":"false"}),b(t.dropdown,{display:"none"}),t.clearActiveOption(),t.refreshState(),i&&t.trigger("dropdown_close",t.dropdown)}positionDropdown(){if("body"===this.settings.dropdownParent){var e=this.control,t=e.getBoundingClientRect(),i=e.offsetHeight+t.top+window.scrollY,s=t.left+window.scrollX
