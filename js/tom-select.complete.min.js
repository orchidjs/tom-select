...
p?Array.isArray(p)?p.push(t):u[s][l]=[p,t]:u[s][l]=t}}else{var c=v(e)
c[n]=c[n]||e.textContent,c[r]=c[r]||s,c[o]=c[o]||e.disabled,c[l]=c[l]||t,u[s]=c,a.push(c),e.selected&&g.items.push(s)}},m=e=>{var t,i;(i=v(e))[p]=i[p]||e.getAttribute("label")||"",i[c]=i[c]||h++,i[o]=i[o]||e.disabled,g.optgroups.push(i),t=i[c]
for(const i of e.children)f(i,t)}
g.maxItems=e.hasAttribute("multiple")?null:1
for(const i of e.children)"optgroup"===(t=i.tagName.toLowerCase())?m(i):"option"===t&&f(i)})():(()=>{var t,o,l,a,d=e.getAttribute(s)
...
e.tomselect=this,i=(window.getComputedStyle&&window.getComputedStyle(e,null)).getPropertyValue("direction"),this.settings=v(e,t),this.input=e,this.tabIndex=e.getAttribute("tabindex")||null,this.is_select_tag="select"===e.tagName.toLowerCase(),this.rtl=/rtl/i.test(i),this.isRequired=e.required,this.settings.load&&this.settings.loadThrottle&&(this.settings.load=(s=this.settings.load,n=this.settings.loadThrottle,function(e,t){var i=this
...
for(n=y.items.length,"number"==typeof f.settings.maxOptions&&(n=Math.min(n,f.settings.maxOptions)),n>0&&(C=!0),g={},r=[],t=0;t<n;t++){let e=f.options[y.items[t].id],n=d(e[f.settings.valueField]),o=f.getOption(n)
for(o||(o=f.render("option",e)),l=e[f.settings.optgroupField]||"",i=0,s=(a=Array.isArray(l)?l:[l])&&a.length;i<s;i++)l=a[i],f.optgroups.hasOwnProperty(l)||(l=""),g.hasOwnProperty(l)||(g[l]=document.createDocumentFragment(),r.push(l)),i>0&&(o=o.cloneNode(!0),b(o,"active")),g[l].appendChild(o)}for(this.settings.lockOptgroupOrder&&r.sort(((e,t)=>(f.optgroups[e]&&f.optgroups[e].$order||0)-(f.optgroups[t]&&f.optgroups[t].$order||0))),p=document.createDocumentFragment(),t=0,n=r.length;t<n;t++)if(l=r[t],f.optgroups.hasOwnProperty(l)&&g[l].children.length){let e=document.createDocumentFragment()
...
if(f.dropdown_content.innerHTML="",f.dropdown_content.appendChild(p),f.settings.highlight&&(f.dropdown_content,v=document.querySelectorAll("span.highlight"),Array.prototype.forEach.call(v,(function(e,t){var i=e.parentNode
...
if(f.loading?I("loading"):0===y.items.length&&f.settings.render.no_results&&m.length&&I("no_results"),(c=f.canCreate(m))&&(h=I("option_create")),f.hasOptions=y.items.length>0||c,C){if(y.items.length>0){if(!(u=w&&f.getOption(w))||!f.dropdown_content.contains(u)){let e=0
