...
if(t.is_select_tag){const e=[],o=t.input.querySelectorAll("option:checked").length
function r(i,s,r){return i||(i=w('<option value="'+N(s)+'">'+N(r)+"</option>")),i!=n&&t.input.append(i),e.push(i),(i!=n||o>0)&&(i.selected=!0),i}t.input.querySelectorAll("option:checked").forEach((e=>{e.selected=!1})),0==t.items.length&&"single"==t.settings.mode?r(n,"",""):t.items.forEach((n=>{if(i=t.options[n],s=i[t.settings.labelField]||"",e.includes(i.$option)){r(t.input.querySelector(`option[value="${Q(n)}"]:not(:checked)`),n,s)}else i.$option=r(i.$option,n,s)}))}else t.input.value=t.getValue()
...
if(i){const s=P(i)
e.setCaret(t>0?s+1:s),e.setActiveItem(),A(i,"last-active")}else e.setCaret(e.caretPos+t)}))})),W.define("dropdown_input",(function(){var e=this
