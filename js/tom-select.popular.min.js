...
if(o=c.settings.mode,r=u.getAttribute("class")||"",F(e=A("<div>"),p.wrapperClass,r,o),F(t=A('<div class="items">'),p.inputClass),e.append(t),F(s=c.render("dropdown"),p.dropdownClass,o),F(n=A(`<div role="listbox" id="${g}" tabindex="-1">`),p.dropdownContentClass),s.append(n),A(p.dropdownParent||e).appendChild(s),p.controlInput)i=A(p.controlInput)
else{i=A('<input type="text" autocomplete="off" size="1" />')
for(const e of["autocorrect","autocapitalize","autocomplete"])u.getAttribute(e)&&q(i,{[e]:u.getAttribute(e)})}p.controlInput||(i.tabIndex=u.disabled?-1:c.tabIndex,t.appendChild(i)),q(i,{role:"combobox",haspopup:"listbox","aria-expanded":"false","aria-controls":g}),a=C(i,c.inputId+"-tomselected")
let f="label[for='"+function(e){return e.replace(/['"\\]/g,"\\$&")}(c.inputId)+"']",v=document.querySelector(f)
if(v){q(v,{for:a}),q(n,{"aria-labelledby":C(v,c.inputId+"-ts-label")})}var m,y,O
c.settings.copyClassesToDropdown&&F(s,r),e.style.width=u.style.width,c.plugins.names.length&&(l="plugin-"+c.plugins.names.join(" plugin-"),F([e,s],l)),(null===p.maxItems||p.maxItems>1)&&c.is_select_tag&&q(u,{multiple:"multiple"}),c.settings.placeholder&&q(i,{placeholder:p.placeholder}),!c.settings.splitOn&&c.settings.delimiter&&(c.settings.splitOn=new RegExp("\\s*"+d(c.settings.delimiter)+"+\\s*")),this.settings.load&&this.settings.loadThrottle&&(this.settings.load=(m=this.settings.load,y=this.settings.loadThrottle,function(e,t){var i=this
O&&(i.loading=Math.max(i.loading-1,0)),clearTimeout(O),O=setTimeout((function(){O=null,i.loadedSearches[e]=!0,m.call(i,e,t)}),y)})),c.control=t,c.control_input=i,c.wrapper=e,c.dropdown=s,c.dropdown_content=n,c.control_input.type=u.type,w(s,"click",(e=>{const t=P(e.target,"[data-selectable]")
t&&(c.onOptionSelect(e,t),b(e,!0))})),w(t,"click",(e=>{var s=P(e.target,"."+c.settings.itemClass,t)
s&&c.onItemSelect(e,s)?b(e,!0):""==i.value&&(c.onClick(e),b(e,!0))})),w(i,"mousedown",(e=>{""!==i.value&&e.stopPropagation()})),w(i,"keydown",(e=>c.onKeyDown(e))),w(i,"keyup",(e=>c.onKeyUp(e))),w(i,"keypress",(e=>c.onKeyPress(e))),w(i,"resize",(()=>c.positionDropdown()),h),w(i,"blur",(e=>c.onBlur(e))),w(i,"focus",(e=>c.onFocus(e))),w(i,"paste",(e=>c.onPaste(e)))
...
w(document,"mousedown",I),w(window,"sroll",_,h),w(window,"resize",_,h),c._destroy=()=>{document.removeEventListener("mousedown",I),window.removeEventListener("sroll",_),window.removeEventListener("resize",_)},this.revertSettings={innerHTML:u.innerHTML,tabIndex:u.tabIndex},u.tabIndex=-1,q(u,{hidden:"hidden"}),u.insertAdjacentElement("afterend",c.wrapper),c.setValue(p.items),delete p.items,w(u,"invalid",(e=>{b(e),c.isInvalid||(c.isInvalid=!0,c.refreshState())})),c.updateOriginalInput(),c.refreshItems(),c.close(!1),c.inputState(),c.isSetup=!0,u.disabled&&c.disable(),c.on("change",this.onChange),F(u,"tomselected"),c.trigger("initialize"),!0===p.preload&&c.load("")}setupOptions(e=[],t=[]){for(const t of e)this.registerOption(t)
...
for(let i=0;i<s.length;i++)if(s[i]==e)return t>0?s[i+1]:s[i-1]}}getItem(e){if(e=m(e))return e=e.replace(/[\\"']/g,"\\$&"),this.control.querySelector(`[data-value="${e}"]`)}addItems(e,t){var i=this
