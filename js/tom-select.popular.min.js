...
var u
if(C(r,n.wrapperClass,d,p),C(l,n.controlClass),Q(r,l),C(a,n.dropdownClass,p),n.copyClassesToDropdown&&C(a,d),C(c,n.dropdownContentClass),Q(a,c),w(n.dropdownParent||r).appendChild(a),n.hasOwnProperty("controlInput"))n.controlInput?(u=w(n.controlInput),this.focus_node=u):(u=w("<input/>"),this.focus_node=l)
...
else{switch(e.keyCode){case 65:if(B(V,e))return void t.selectAll()
break
...
if(v.loading?I("loading"):v.settings.shouldLoad.call(v,f)?0===m.items.length&&I("no_results"):I("not_loading"),(a=v.canCreate(f))&&(p=I("option_create")),v.hasOptions=m.items.length>0||a,b){if(m.items.length>0){if(!w.contains(O)&&"single"===v.settings.mode&&v.items.length&&(O=v.getOption(v.items[0])),!w.contains(O)){let e=0
