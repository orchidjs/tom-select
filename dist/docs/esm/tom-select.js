...
class TomSelect extends MicroPlugin(MicroEvent) {
  ...
  removeItem(value = null, silent) {
    const self = this;
    const item = self.getItem(value);
    if (!item) return;
    var i, idx;
    const hashed = item.dataset.value;
    i = nodeIndex(item);
    item.remove();
  ...
  deleteSelection(e) {
    ...
    console.log('delete selection'); // determine items that will be removed
    ...
    if (self.activeItems.length) {
      ...
      const _items = self.controlChildren();

      if (direction < 0 && selection.start === 0 && selection.length === 0) {
        values.push(self.items[self.caretPos - 1]);

        _items.push(_items[self.caretPos - 1]);
      } else if (direction > 0 && selection.start === self.inputValue().length) {
        values.push(self.items[self.caretPos]);

        _items.push(_items[self.caretPos]);
      }
