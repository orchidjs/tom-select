...
# update package.json
if ! sed -i.bak 's/"version": "[^"]*"/"version": "'$VERSION'"/' package.json; then
	echo 'version not replaced in package.json'
	exit
fi

if ! rm package.json.bak; then
	echo 'could not remove package.json.bak'
	exit
fi
